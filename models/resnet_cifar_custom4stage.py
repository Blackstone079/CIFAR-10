...
class ResNet(nn.Module):
    ...
    def __init__(self):
        super().__init__()  
        ch_in = 3
        self.conv1 = nn.Conv2d(in_channels=ch_in, out_channels=16, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(num_features=16)
        self.relu = nn.ReLU(inplace=False)
        self.ch_current = 16

        self.layer1 = self._make_stage(ch_out=16, n_blocks=3, stride=1)
        self.layer2 = self._make_stage(ch_out=32, n_blocks=3, stride=2)
        self.layer3 = self._make_stage(ch_out=64, n_blocks=3, stride=2)
        self.layer4 = self._make_stage(ch_out=128, n_blocks=3, stride=2)
    ...
    def _make_stage(self, ch_out, n_blocks, stride):
        blocks = []
        block1 = BasicBlock(self.ch_current, ch_out, stride)
